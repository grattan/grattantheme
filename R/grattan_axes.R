...
#' @param expand_left default is 0. This will ensure your y-axis is at the lowest value of your plotted value.
#' @param expand_right default is 0.015. This will ensure that a small amount of white space is added to the right of your chart.
...
#' # upper limit as usual based on the data. All the usual arguments of grattan_y_continuous
...
grattan_y_continuous <- function(expand_bottom = 0, expand_top = 0.015, ...) {
  if(packageVersion("ggplot2") < '3.2.1.900') {
    ...
    ggplot2::scale_y_continuous(expand = expansion(mult = c(expand_bottom, expand_top)), ...)
...
scale_y_continuous_grattan <- function(expand_bottom = 0, expand_top = 0.015, ...) {
  grattan_y_continuous(expand_bottom = expand_bottom, expand_top = expand_top, ...)
}
...
grattan_x_continuous <- function(expand_left = 0, expand_right = 0.015, ...) {
  if(packageVersion("ggplot2") < '3.2.1.900') {
    ...
    ggplot2::scale_x_continuous(expand = expand_scale(mult = c(expand_bottom, expand_top)), ...)

  } else {

    ggplot2::scale_x_continuous(expand = expansion(mult = c(expand_bottom, expand_top)), ...)
...
scale_x_continuous_grattan <- function(expand_left = 0, expand_right = 0.015, ...){
  grattan_x_continuous(expand_left = expand_left, expand_right = expand_right, ...)
}

...
* Deprecated chart types are now treated consistently across use-cases. Only `fullslide_old` and `fullslide_43` are still usable as deprecated types, because they are the only ones with a surviving Powerpoint template. Removed stale references to deprecated chart types from the `grattan_save()` documentation.
